...
class SimpleCaptchaHandlerController extends AppController
{
    ...
    public function getScriptInclude()
    {
        ...
        if ($this->SimpleCaptcha->RemoteScriptEnabled) {
            $script .= "\r\n";
            $script .= \BDC_SimpleCaptchaScriptsHelper::GetRemoteScript($this->SimpleCaptcha, $this->getClientSideFramework());
        }
    ...
    {
        $clientSide = $this->request->getQuery('cs');
    ...
    private function getCaptchaId()
    {
        $captchaId = $this->request->getQuery('t');
        if (
            !\BDC_StringHelper::HasValue($captchaId) ||
